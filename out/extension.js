...
const updateStatusBar = (timerItem, startButton, pauseButton, resetButton) => {
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('project-timer.start', () => {
        if (!isPaused)
            return;
        isPaused = false;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('project-timer.pause', () => {
        if (isPaused)
            return;
        isPaused = true;
        if (timerInterval) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('project-timer.reset', () => {
        elapsedSeconds = 0;
        if (isPaused) {
            updateStatusBar(timerItem, startButton, pauseButton, resetButton);
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('project-timer.showStats', () => {
        const statStrings = stats.map((stat) => `${stat.date} - ${stat.projectName}: ${formatTime(stat.timeSpent)}`);
